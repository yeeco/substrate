...
#[cfg_attr(feature = "std", derive(Debug))]
#[derive(Default, Clone, Encode, Decode)]
pub struct MultiLayerProof32 {
    pub layer2_merkle: Option<MerkleTree32<ProofHash<BlakeTwo256>, ProofAlgorithm<BlakeTwo256>>>,
    pub layer2_proof: Option<Vec<u8>>,
    layer1_merkles: Vec<(u16, Option<MerkleTree32<ProofHash<BlakeTwo256>, ProofAlgorithm<BlakeTwo256>>>)>,
}

impl MultiLayerProof32 {
    /// Turns a MultiLayerProof into the raw bytes.
    pub fn into_bytes(&self) -> Vec<u8> {
        self.encode()
    }
    ...
    pub fn to_MultiLayerProof(self) -> MultiLayerProof {
        let layer2_merkle = self.layer2_merkle.map(|mt32| mt32.to_MerkleTree());
        let layer2_proof = self.layer2_proof;
        let layer1_merkles = self.layer1_merkles.into_iter().map(|(shard, item)| (shard, item.map(|mt32| mt32.to_MerkleTree()))).collect();
        MultiLayerProof{
            layer2_merkle,
            layer2_proof,
            layer1_merkles,
